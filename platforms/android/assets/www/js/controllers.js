...
.controller('cadastrarObjetoCtrl', function ($scope, $state, $http,$cordovaGeolocation, $ionicLoading, $ionicPlatform) {
  $scope.selectedCategory = null;
      ...
      $http.get('http://10.10.29.135:8080/MobileDevices/MyServer?category=all').success(function(data) {
    ...
    console.log("aqui mapa chamado");
      ...
      var data = {name: 'pedro',
        ...
        $scope.addObject= function(){
          $http({
            url: 'http://10.10.29.135:8080/MobileDevices/MyServer',
            method: 'POST',
              params: data,
            crossDomain: true,
        headers: {
...
.controller('loginCtrl', function ($scope, $http, $state, LoggedUser ,$log) {
  ...
  $scope.doLogin = function() {
    $http.get('http://10.10.29.135:8080/MobileDevices/MyServer?username='+$scope.user+'&password='+$scope.pass).success(function(data){
        $scope.$log = $log;
        if (data !=null){
